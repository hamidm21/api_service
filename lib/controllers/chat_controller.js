...
exports.getMessages = async (req, res, next) => {
	...
	try {
		log(req.body);
		const valid = validator.joi.validate(req.body , validator.getMessages);
		if (valid.error)
			...
			const result = await Message.getMessages(room_id, limit, user_id, message_id);
			log({'asdasdasdasdsda':  result});
...
exports.readMessage = async (message) => {
	...
	try {
		...
		if (valid.error) {
			...
			const isRead = await Message.readMessages(msg.message_id, message.highWaterOffset);
			log(isRead);
		}
	} catch (e) {
		winston.error(`error in  - ${e.status || 500} - ${e.message} - ${e.stack} - ${new Date()}`);
