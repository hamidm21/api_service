	...
	try {
		...
		if (valid.error) {
			...
			const automatedMessage = Object.assign({}, config.AUTOMATED_MESSAGE, {
				text: room_members[0].type === 'Secretary' ? new Date().getHours() > 1 && new Date().getHours() < 8 ? await config.SECRETARY_NIGHT_MSG(room_members[0].username) : await config.SECRETARY_MSG(room_members[0].username) : room_members[0].type === 'Psychologist' ? config.PSYCHOLOGIST_MSG(room_members[0].username) : null,
				type: 'text',
				room_id,
			...
			log({lastMessage});
			const saved = await Room.createRoom(room_id, room_members[0].user_id, user_id, room_members, user_id, room_members[0].user_id);
			log({saved});
