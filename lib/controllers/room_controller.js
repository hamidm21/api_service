...
const elog = require('debug')('api:error:controller:room');
...
exports.createRoom = async (req, res, next) => {
	...
	try {
		...
		if (valid.error) {
			...
			const room_members = typeof members === 'object' ?
			...
			const saved = await Room.createRoom(room_id, room_members[0].user_id, user_id, room_members, user_id, room_members[0].user_id, members ? members.length !== 0 ? false : false : true);
			log({
				saved
			});
...
exports.notNew = async message => {
	...
	try {
		...
		if (valid.error) {
			...
			const updated = await Room.setIsNewToFalse(msg.room_id);
			log({
